...
my $operators = {
  'LSHIFT'  => { operators => 2, fn => sub { return $_[0] << $_[1]} },
  'RSHIFT'  => { operators => 2, fn => sub { return $_[0] >> $_[1]} },
  'AND' => { operators => 2, fn => sub { return $_[0] & $_[1] } },
  'OR'  => { operators => 2, fn => sub { return $_[0] | $_[1] } },
  'XOR' => { operators => 2, fn => sub { return $_[0] ^ $_[1] } },
  'NOT' => { operators => 1, fn => sub { return ~$_[0] } }
};
...
sub value
{
  ...
  if (defined ($target->{op}))
  {
    ...
    if (defined $stack[1])
    {
      $ret = $operators->{$target->{op}}{fn}($stack[0], $stack[1]);
    }
    else
    {
      $ret = $operators->{$target->{op}}{fn}($stack[0]);
    }
